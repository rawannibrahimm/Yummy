...
async function getAreaMeals(areaName) {
    ...
    const start = Date.now();
    try {
        let areaMealsResponse = await fetch(`https://www.themealdb.com/api/json/v1/1/filter.php?a=${areaName}`);
        let data = await areaMealsResponse.json();
        // console.log(data);
        let meals = data.meals;
        currentMeals = meals;
        // console.log(currentMeals);
...
async function getIngredientMeals(ingredientName) {
    ...
    const start = Date.now();
    try {
        let ingredientMealsResponse = await fetch(`https://www.themealdb.com/api/json/v1/1/filter.php?i=${ingredientName}`);
        let data = await ingredientMealsResponse.json();
        // console.log(data);
        let meals = data.meals;
        let firstTwentyMeals = meals.slice(0, 20);
        currentMeals = firstTwentyMeals;
